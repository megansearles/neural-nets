##This is an attempt to recreate the simple.py using just tensorflow
...
def softmax_regression() :
    ...
    #reduce sum and reduce mean compute mean and sume. I think the reduce means that it collapse it along an axis
...
def efftest() :
    ...
    input = tf.placeholder(tf.float32, [None, 3, 256, 256])
    labels = tf.placeholder(tf.float32, [None, 11])

    with tf.name_scope('conv11') :
        l1_weights = create_weight([3,3,256,32])
        l1_bias = create_bias([32])
    ...
    with tf.name_scope('fc2') :
        ...
        fc2_input = tf.reshape(l1_pool, [-1, 2 * 128 * 32])
        fc2_weights = create_weight([2 * 128 * 32, 30])
    ...
    run_all_summaries = tf.merge_all_summaries()
    sess = tf.Session() 
    sess.run(tf.initialize_all_variables())
    log_writer = tf.train.SummaryWriter('/tmp/tenflow_log', sess.graph)

    for i in range(3000) :
        batch_start = (i % 10) * 70
        summary, _ = sess.run([run_all_summaries, back_prop], feed_dict={input:data['train_image'][batch_start:batch_start + 70], labels:data['train_label'][batch_start: batch_start + 70]})
        log_writer.add_summary(summary, i)

        if (i % 180 == 0) :
            summary, current_accuracy = sess.run([run_all_summaries, accuracy], feed_dict={input:data['test_image'], labels:data['test_label']})
